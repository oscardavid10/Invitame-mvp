...
import { resolveTemplate } from "../utils/templates.js";
const router = Router();
...
router.get("/", authed, async (req, res) => {
  ...
  const msg = total
    ? "Aún no tienes invitaciones activas."
    : "Compra un plan para continuar.";
...
router.post("/wizard/sections", authed, async (req, res) => {
  ...
  await req.db.query(
    "UPDATE invitations SET section_order=? WHERE id=? AND user_id=?",
    [JSON.stringify(arr), invitation_id, req.session.uid]
  );
...
router.post("/wizard/template", authed, async (req, res) => {
  ...
  await req.db.query(
    "UPDATE invitations SET template_key=? WHERE id=? AND user_id=?",
    [template_key, invitation_id, req.session.uid]
  );
  if (req.headers.accept === "application/json") return res.json({ ok: true });
  res.redirect("/panel/wizard");
});
...
router.post("/wizard/date", authed, async (req, res) => {
  ...
  const [[inv]] = await req.db.query(
    "SELECT date_locked FROM invitations WHERE id=? AND user_id=?",
  ...
  if (inv.date_locked)
    return res
      .status(400)
      .send("La fecha ya está bloqueada y no puede cambiarse");
  await req.db.query(
    "UPDATE invitations SET date_iso=?, date_locked=1, slug_locked=1 WHERE id=? AND user_id=?",
    [date_iso, invitation_id, req.session.uid]
  );
  if (req.headers.accept === "application/json") return res.json({ ok: true });
  res.redirect("/panel");
});
...
router.post("/wizard/slug", authed, async (req, res) => {
  ...
  const [[inv]] = await req.db.query(
    "SELECT slug_locked FROM invitations WHERE id=? AND user_id=?",
    [invitation_id, req.session.uid]
  );
  if (!inv) return res.status(404).send("Invitación no encontrada");
  if (inv.slug_locked)
    return res.status(400).send("El slug/URL ya está bloqueado");
  try {
    await req.db.query(
      "UPDATE invitations SET slug=? WHERE id=? AND user_id=?",
      [slug, invitation_id, req.session.uid]
    );
    ...
    if (e.code === "ER_DUP_ENTRY")
      return res.status(400).send("Ese slug ya existe");
    throw e;
  }
});
...
router.post("/wizard/theme", authed, async (req, res) => {
  ...
  let incoming = {};
  try {
    incoming = JSON.parse(theme_json);
  } catch {}
  const [[inv]] = await req.db.query(
    "SELECT theme_json FROM invitations WHERE id=? AND user_id=?",
    [invitation_id, req.session.uid]
  );
  const current = (() => {
    try {
  ...
  await req.db.query(
    "UPDATE invitations SET theme_json=? WHERE id=? AND user_id=?",
    [JSON.stringify(merged), invitation_id, req.session.uid]
  );
  if (req.headers.accept === "application/json") return res.json({ ok: true });
  res.redirect("/panel");
});
...
router.post("/publish", authed, async (req, res) => {
  ...
  await req.db.query(
    'UPDATE invitations SET status="active", published_at=NOW() WHERE id=? AND user_id=?',
    [invitation_id, req.session.uid]
  );
  res.redirect("/panel");
});
...
router.get("/preview/:id", authed, async (req, res) => {
  ...
  theme.meta = {
    ...
    show_ceremony_map:
      theme.meta?.show_ceremony_map ?? inv.show_ceremony_map ?? false,
  };
  const view = resolveTemplate(inv.template_key);
