    ...
    try {
      ...
      if (event.type === "checkout.session.completed") {
        ...
        const [[exists]] = await pool.query(
          "SELECT id FROM invitations WHERE order_id=?",
          [orderId]
        );
        if (!exists) {
          ...
          const [[tpl]] = await pool.query(
            "SELECT * FROM templates WHERE key_name=? LIMIT 1",
            [template_key]
          );
          const base = tpl?.demo_theme_json
            ...
            : {
                ...
                media: {
                  video: "/public/video/sample.mp4",
                  poster: "/public/img/placeholder.jpg",
                  gallery: [],
                },
...
app.get("/u/:slug", async (req, res) => {
  const [rows] = await req.db.query("SELECT * FROM invitations WHERE slug=?", [
    req.params.slug,
  ]);
...
cron.schedule("15 3 * * *", async () => {
  try {
    ...
    if (toArchive.length) {
      await pool.query(
        `UPDATE invitations SET status="archived", archived_at=NOW() WHERE id IN (${toArchive
          .map(() => "?")
          .join(",")})`,
        toArchive
      );
    ...
    console.error("Cron archive err:", e.message);
  }
});
